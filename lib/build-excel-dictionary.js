...
const tableComments = require("./table-comments.json");
...
const DATA_COLS = ["Column name", "Type", "NULL", "Description", "Index"];
...
function formatDictionaryDate(date) {
  const d = new Date(date);
...
function nullFromNullable(nullable) {
  const n = String(nullable ?? "").toLowerCase().trim();
...
const TABLE_COMMENT_MAP = tableComments;
...
function tableCommentFromRows(rows) {
  for (const row of rows) {
    const comment = cleanSingleLine(get(row, "Table Comment", "table_comment", "table_description"));
    if (comment && comment !== "-") return comment;
  }
  return null;
}

function tableCommentFromMapping(tableName) {
...
function formatTableLabel(tableName, rows) {
  const description = tableCommentFromRows(rows) ?? tableCommentFromMapping(tableName) ?? "-";
  return `${tableName} (${description})`;
}

function tableMetaFromRows(rows, ...keys) {
  for (const row of rows) {
...
function tableVersionFromRows(rows) {
  return tableMetaFromRows(rows, "Table Version", "table_version");
}

function tableVersionLabel(version) {
...
function tableNamesForAudience(tableNames, byTable, audience) {
...
function rowToDictionaryCells(row) {
  const colName = get(row, "Column Name", "column_name");
...
function applyDictionaryTitleStyle(row) {
  for (let c = 1; c <= 5; c++) {
    const cell = row.getCell(c);
    cell.border = THIN_BORDER;
...
function applyInfoRowStyle(row) {
  for (let c = 1; c <= 5; c++) {
    const cell = row.getCell(c);
    cell.border = THIN_BORDER;
...
function writeDictionarySections(sheet, tableNames, byTable, dateStr) {
  sheet.columns = [
    { width: 28 },
    { width: 18 },
    { width: 10 },
  ...
  for (const tableName of tableNames) {
    ...
    const tableComment = tableCommentFromRows(tableRows) ?? tableCommentFromMapping(tableName) ?? "-";
...
function setAutoColumnWidthsScanAllRows(sheet, numCols) {
  for (let c = 1; c <= numCols; c++) {
    let maxLen = 10;
...
async function buildWorkbook(rows, databaseName) {
  ...
  const humanSheet = workbook.addWorksheet("Human Tables", { views: [{ showGridLines: true }] });
  ...
  const animalSheet = workbook.addWorksheet("Animal Tables", { views: [{ showGridLines: true }] });
  writeDictionarySections(
    animalSheet,
