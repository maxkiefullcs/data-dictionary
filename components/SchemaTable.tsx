...
function AdminTable({
  ...
  return (
    <div className="schema-table-wrap min-h-0 min-w-0 overflow-auto rounded-lg border border-navy-600">
      <table className="w-full border-collapse" style={{ minWidth: "72rem" }}>
        <colgroup>
        ...
        <thead>
          <tr>
            {showTableColumn && (
              <th className="sticky-left-0 w-0 px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-300 whitespace-nowrap">
                Table
              </th>
            )}
            <th
              className={
                showTableColumn
                  ? "sticky-left-1 w-0 px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-300 whitespace-nowrap"
                  : "sticky-left-0 w-0 px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-300 whitespace-nowrap"
              }
            >
              Column Name
            </th>
            <th className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-300 whitespace-nowrap">
              Data Type
            </th>
            <th className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-300 whitespace-nowrap">
              Length
            ...
            <th className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-300">
              Comment
            </th>
          </tr>
        </thead>
        <tbody>
          {rows.map((row, i) => (
            ...
            >
              {showTableColumn && (
                <td className="sticky-left-0 border-b border-navy-700/80 px-4 py-2.5 align-top font-mono text-sm text-slate-300 whitespace-nowrap">
                  {row.table_name}
                </td>
              )}
              <td
                className={
                  showTableColumn
                    ? "sticky-left-1 border-b border-navy-700/80 px-4 py-2.5 align-top font-mono text-sm font-medium text-slate-100 whitespace-nowrap"
                    : "sticky-left-0 border-b border-navy-700/80 px-4 py-2.5 align-top font-mono text-sm font-medium text-slate-100 whitespace-nowrap"
                }
              >
                {row.column_name}
              </td>
              <td className="border-b border-navy-700/80 px-4 py-2.5 align-top font-mono text-sm text-slate-300 whitespace-nowrap">
                {row.data_type}
              </td>
              <td className="border-b border-navy-700/80 px-4 py-2.5 align-top font-mono text-sm text-slate-400 whitespace-nowrap">
                {dash(row.character_maximum_length)}
              </td>
              <td className="border-b border-navy-700/80 px-4 py-2.5 align-top font-mono text-sm text-slate-400 whitespace-nowrap">
                {dash(row.numeric_precision)}
              </td>
              <td className="border-b border-navy-700/80 px-4 py-2.5 align-top whitespace-nowrap">
                <NullableBadge isNullable={row.is_nullable} />
              </td>
              <td className="border-b border-navy-700/80 px-4 py-2.5 align-top font-mono text-sm text-slate-300 min-w-0 whitespace-pre-wrap break-words">
                {dash(row.column_default)}
              </td>
              <td className="border-b border-navy-700/80 px-4 py-2.5 align-top text-sm text-slate-200 min-w-0 whitespace-pre-wrap break-words leading-relaxed">
